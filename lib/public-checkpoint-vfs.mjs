...
export function opaqueCheckpointKey(checkpoint) {
  ...
  return `${tail.slice(0, 32).toLowerCase()}${Number(checkpoint.toEvent).toString(36).padStart(4, '0')}`;
}
...
export function ingestPublicCheckpointEvent(event, options = {}) {
  ...
  queues.set(ledgerId, run.finally(() => { if (queues.get(ledgerId) === run) queues.delete(ledgerId); }));
  return run;
}
