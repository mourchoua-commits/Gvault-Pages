...
const SCOUT_ACK_DIR='essai/control-tower/public-scout/ack/history';
const SCOUT_STATE='essai/control-tower/public-scout/data/latest.json';
const sha=b=>crypto.createHash('sha256').update(b).digest('hex');
function stable(v){if(Array.isArray(v))return v.map(stable);if(v&&typeof v==='object')return Object.fromEntries(Object.keys(v).sort().map(k=>[k,stable(v[k])]));return v}
const canonical=v=>JSON.stringify(stable(v));
function gitResult(args,{buffer=false}={}){const r=spawnSync('git',args,{cwd:ROOT,encoding:buffer?null:'utf8'});if(r.status!==0)throw new Error(String(r.stderr||r.stdout));return r.stdout;}
const git=args=>String(gitResult(args)||'').trim();
...
function showBuffer(commit,rel){const out=gitResult(['show',`${commit}:${rel}`],{buffer:true});return Buffer.isBuffer(out)?out:Buffer.from(out||'')}

let assertions=0;const ok=(v,m)=>{assert.ok(v,m);assertions+=1};
const requestFiles=await jsonFiles(REQUEST_DIR),messageFiles=await jsonFiles(MESSAGE_DIR),outboundFiles=await jsonFiles(OUTBOUND_DIR),scoutAckFiles=await jsonFiles(SCOUT_ACK_DIR);
ok(requestFiles.length>=1,'request history required');ok(messageFiles.length>=2,'message history expected');ok(outboundFiles.length>=1,'outbound history required');ok(scoutAckFiles.length>=2,'scout result ACK history expected');
...
for(const name of scoutAckFiles){
  const rel=`${SCOUT_ACK_DIR}/${name}`,ackBytes=await raw(rel),ack=JSON.parse(ackBytes.toString('utf8')),dataCommit=path.basename(name,'.json').toLowerCase();
  const {ackDigest,...ackCore}=ack;ok(/^[a-f0-9]{40}$/i.test(dataCommit)&&ack.dataCommitSha===dataCommit,'scout ACK filename/commit binding');ok(ackDigest===sha(Buffer.from(canonical(ackCore),'utf8')),'scout ACK digest');ok(ack.authority==='PUBLIC_ACK_REFERENCES_EXACT_DATA_COMMIT'&&ack.rawBodyPublished===false&&ack.privateDataPublished===false,'scout ACK policy');
  const stateBytes=showBuffer(dataCommit,SCOUT_STATE),state=JSON.parse(stateBytes.toString('utf8')),stateCore={...state};delete stateCore.publicStateSha256;ok(state.publicStateSha256===sha(Buffer.from(canonical(stateCore),'utf8')),'scout state hash at exact commit');ok(ack.publicStateSha256===state.publicStateSha256&&ack.translationDigest===state.translationDigest&&ack.sourceBodySha256===state.sourceBodySha256,'scout ACK/state binding');
  events.push({ingressEventId:`PUBLIC_SCOUT_RESULT:${dataCommit}`,role:'external',surface:'public_data',sha256:sha(stateBytes),bytes:stateBytes.length,commit:dataCommit,ackCommit:provenance(rel),rel:`${SCOUT_STATE}@${dataCommit}`});
}
ok(new Set(events.map(e=>e.ingressEventId)).size===events.length,'all semantic ingress IDs unique');
const latestRequest=await raw('essai/control-tower/public-scout/requests/current.json');ok(requestFiles.some(name=>name===`${sha(latestRequest)}.json`),'request current points to exact history bytes');
...
console.log(JSON.stringify({schema:'GVAULT_PUBLIC_EVENT_HISTORY_AUDIT_V2',status:'PASS',assertions,eventCount:events.length,requestCount:requestFiles.length,messageCount:messageFiles.length,scoutResultCount:scoutAckFiles.length,outboundCount:outboundFiles.length,events:events.map((e,i)=>({observedSequenceCandidate:i+1,...e}))},null,2));
