...
const GUARD_URL=new URL('gthink-throughput-guard.js?v=1',SCRIPT_BASE).href;
...
function ensureGuard(){if(window.GTHINK_THROUGHPUT_GUARD)return Promise.resolve(window.GTHINK_THROUGHPUT_GUARD);if(guardLoad)return guardLoad;guardLoad=new Promise(resolve=>{const s=document.createElement('script');s.src=GUARD_URL;s.async=false;s.dataset.gthinkThroughputGuard='V1';s.onload=()=>resolve(window.GTHINK_THROUGHPUT_GUARD||null);s.onerror=()=>resolve(null);(document.head||document.documentElement).appendChild(s)}).finally(()=>{guardLoad=null});return guardLoad}
