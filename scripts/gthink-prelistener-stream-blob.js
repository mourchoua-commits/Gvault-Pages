(()=>{'use strict';
const SCHEMA='GTHINK_PRELISTENER_STREAM_BLOB_V3';
...
const MINI_URL=new URL('gthink-mini-listener-swarm.js?v=1',SCRIPT_BASE).href;
let attached=false,downstreamReadyAt=0,heartbeat=null,miniLoad=null;
...
function attach(){if(attached)return true;const a=api();if(!a?.speak||!a?.listen)return false;void ensureMini();attachChannels();a.listen(onBlob);for(const b of a.hearLast?.(48)||[])onBlob(b);announce();heartbeat=setInterval(announce,7000);attached=true;window.GTHINK_PRELISTENER_STREAM_BLOB=Object.freeze({schema:SCHEMA,name:NAME,mode:'stream-before-listener',get downstreamReady(){return downstreamReady()},get miniSwarmReady(){return !!window.GTHINK_MINI_LISTENER_SWARM},get queued(){return queue.size},flush:replayQueued});return true}
