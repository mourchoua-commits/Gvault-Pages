(()=>{'use strict';
window.GTHINK_OFFLINE_ONLY=true;
const SCHEMA='GTHINK_PRELISTENER_STREAM_BLOB_V7_OFFLINE';
...
function speak(kind,payload={},parentBlobId=null,meta={}){const a=api();if(!a?.speak)return null;return a.speak({schema:BLOB_SCHEMA,blobId:uid(),parentBlobId,conversationId:meta.conversationId||payload?.conversationId||'gthink-prelistener',kind,role:meta.role||'prelistener',from:meta.from||NAME,to:meta.to||'public.bus',intent:meta.intent||'prelisten_stream',language:'fr',at:new Date().toISOString(),surface:'Gvault-Pages',streamUrl:a.streamUrl,text:meta.text,payload:{...payload,schema:SCHEMA,offlineOnly:true},understoodBy:['GThinkPrelistener','GThinkMini','GThink','public-kernel','public-ui','throughput-guard'],silent:true,muted:false})}
function announce(){speak('gthink.prelistener.ready',{state:'prelistener_ready',name:NAME,mode:'offline-stream-before-listener',downstreamReady:downstreamReady(),miniSwarmReady:!!window.GTHINK_MINI_LISTENER_SWARM,throughputGuardReady:!!window.GTHINK_THROUGHPUT_GUARD,queued:queue.size,offlineOnly:true,streamUrl:api()?.streamUrl||null},null,{role:'prelistener',from:NAME,to:'public.bus',intent:'announce_prelistener_ready',text:'GThink prelistener offline ready'})}
...
function attach(){if(attached)return true;const a=api();if(!a?.speak||!a?.listen)return false;attachChannels();a.listen(onBlob);for(const b of a.hearLast?.(48)||[])onBlob(b);announce();heartbeat=setInterval(announce,5000);attached=true;window.GTHINK_PRELISTENER_STREAM_BLOB=Object.freeze({schema:SCHEMA,name:NAME,mode:'offline-stream-before-listener',offlineOnly:true,get downstreamReady(){return downstreamReady()},get miniSwarmReady(){return !!window.GTHINK_MINI_LISTENER_SWARM},get throughputGuardReady(){return !!window.GTHINK_THROUGHPUT_GUARD},get queued(){return queue.size},flush:replayQueued});return true}
