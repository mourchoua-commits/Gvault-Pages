(()=>{'use strict';
const SCHEMA='GTHINK_PRELISTENER_STREAM_BLOB_V6';
...
const PARSER_MODE=document.readyState==='loading';
function parserLoad(url,attr,value){if(!PARSER_MODE||document.querySelector(`[${attr}]`))return false;document.write(`<script src="${url}" ${attr}="${value}"><\/script>`);return true}
const MINI_PARSER=parserLoad(MINI_URL,'data-gthink-mini-listener-swarm','V2');
const GUARD_PARSER=parserLoad(GUARD_URL,'data-gthink-throughput-guard','V2');
...
function waitGlobal(name,ms=2500){const start=Date.now();return new Promise(resolve=>{const tick=()=>{if(window[name])return resolve(window[name]);if(Date.now()-start>=ms)return resolve(null);setTimeout(tick,20)};tick()})}
function ensureMini(){if(window.GTHINK_MINI_LISTENER_SWARM)return Promise.resolve(window.GTHINK_MINI_LISTENER_SWARM);if(MINI_PARSER)return waitGlobal('GTHINK_MINI_LISTENER_SWARM');if(miniLoad)return miniLoad;miniLoad=new Promise(resolve=>{const s=document.createElement('script');s.src=MINI_URL;s.async=false;s.dataset.gthinkMiniListenerSwarm='V2';s.onload=()=>resolve(window.GTHINK_MINI_LISTENER_SWARM||null);s.onerror=()=>resolve(null);(document.head||document.documentElement).appendChild(s)}).finally(()=>{miniLoad=null});return miniLoad}
function ensureGuard(){if(window.GTHINK_THROUGHPUT_GUARD)return Promise.resolve(window.GTHINK_THROUGHPUT_GUARD);if(GUARD_PARSER)return waitGlobal('GTHINK_THROUGHPUT_GUARD');if(guardLoad)return guardLoad;guardLoad=new Promise(resolve=>{const s=document.createElement('script');s.src=GUARD_URL;s.async=false;s.dataset.gthinkThroughputGuard='V2';s.onload=()=>resolve(window.GTHINK_THROUGHPUT_GUARD||null);s.onerror=()=>resolve(null);(document.head||document.documentElement).appendChild(s)}).finally(()=>{guardLoad=null});return guardLoad}
...
function attach(){if(attached)return true;const a=api();if(!a?.speak||!a?.listen)return false;if(!PARSER_MODE)void ensureChain();attachChannels();a.listen(onBlob);for(const b of a.hearLast?.(48)||[])onBlob(b);announce();heartbeat=setInterval(announce,5000);attached=true;window.GTHINK_PRELISTENER_STREAM_BLOB=Object.freeze({schema:SCHEMA,name:NAME,mode:'stream-before-listener',parserMode:PARSER_MODE,get downstreamReady(){return downstreamReady()},get miniSwarmReady(){return !!window.GTHINK_MINI_LISTENER_SWARM},get throughputGuardReady(){return !!window.GTHINK_THROUGHPUT_GUARD},get queued(){return queue.size},flush:replayQueued,ensureChain});return true}
