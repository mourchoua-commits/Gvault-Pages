...
const CORE_URL=new URL('gthink-page-transport-core.js?v=2',document.currentScript?.src||location.href).href;
let coreLoad=null;
...
function load(){const c=core();if(c?.askRequest)return Promise.resolve(c);if(coreLoad)return coreLoad;coreLoad=new Promise(resolve=>{const existing=document.querySelector('script[data-gvault-page-transport-core="V2"]');if(existing){waitForCore(resolve,Date.now());return}const s=document.createElement('script');s.src=CORE_URL;s.async=false;s.setAttribute('data-gvault-page-transport-core','V2');s.onload=()=>waitForCore(resolve,Date.now());s.onerror=()=>resolve(null);(document.head||document.documentElement).appendChild(s)}).finally(()=>{coreLoad=null});return coreLoad}
