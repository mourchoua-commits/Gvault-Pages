(()=>{'use strict';
const SCHEMA='GTHINK_PUBLIC_NATIVE_ENGINE_V3_CORE_COGNITION';
...
const CONVERSATION_URL=new URL('gthink-secondary-conversation-bridge.js?v=2',SCRIPT_BASE).href;
...
const readyPromise=Promise.all([
  ...
  loadScript(CONVERSATION_URL,'data-gthink-secondary-conversation-bridge-v2')
]).then(()=>{
...
async function answer(request){const {core,conversation}=await readyPromise;const context=request?.payload?.secondKernelContext||null;let conversational=null;try{conversational=conversation?.answer?.(request,context)||null}catch{}if(conversational?.handled&&String(conversational?.text||'').trim())return {schema:SCHEMA,handled:true,text:String(conversational.text),engine:'gthink-public-native-js+core-cognition',model:'core-cognition-conversation-v2',publicNative:true,offlineCapable:true,actionsAuthorized:false,conversationIntent:conversational.intent||null,evidence:conversational.evidence||[],knowledgeBranch:conversational.knowledgeBranch||null,knowledgeAware:true,coreCognitionAware:true};const result=await core.answer(request);return typeof result==='string'?{schema:SCHEMA,handled:true,text:result,engine:'gthink-public-native-js',model:'native-rules-v1',publicNative:true,offlineCapable:true,actionsAuthorized:false,knowledgeAware:false}:{...result,schema:result?.schema||SCHEMA,knowledgeAware:false}}
function status(){let base={configured:true,ready:false,mode:'public-native-loading',engine:'gthink-public-native-js+core-cognition',model:'core-cognition-conversation-v2',offlineCapable:true,networkRequired:false,knowledgeAware:true,coreCognitionAware:true};try{if(coreRef?.status)base={...base,...coreRef.status(),ready:true,engine:'gthink-public-native-js+core-cognition',model:'core-cognition-conversation-v2',knowledgeAware:true,coreCognitionAware:true,conversationBridge:!!conversationRef,conversationBridgeSchema:conversationRef?.schema||null}}catch{}return base}
wrapper=Object.freeze({schema:SCHEMA,name:'GThinkPublicNativeCoreCognition',answer,status,get core(){return coreRef},get conversation(){return conversationRef},ready:()=>readyPromise});
window.GTHINK_PUBLIC_NATIVE_ENGINE=wrapper;
})();