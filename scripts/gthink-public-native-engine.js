(()=>{'use strict';
const SCHEMA='GTHINK_PUBLIC_NATIVE_ENGINE_V5_SIM_TRAINED';
...
async function answer(request){const {core,conversation}=await readyPromise;const context=request?.payload?.secondKernelContext||null;let conversational=null;try{conversational=conversation?.answer?.(request,context)||null}catch{}if(conversational?.handled&&String(conversational?.text||'').trim())return {schema:SCHEMA,handled:true,text:String(conversational.text),engine:'gthink-public-native-js+core-cognition+co-development+sim-trained',model:'simulation-trained-conversation-v4',publicNative:true,offlineCapable:true,actionsAuthorized:false,conversationIntent:conversational.intent||null,evidence:conversational.evidence||[],knowledgeBranch:conversational.knowledgeBranch||null,knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true,simulationTrained:true};const result=await core.answer(request);return typeof result==='string'?{schema:SCHEMA,handled:true,text:result,engine:'gthink-public-native-js',model:'native-rules-v1',publicNative:true,offlineCapable:true,actionsAuthorized:false,knowledgeAware:false,coDevelopmentAware:true,simulationTrained:true}:{...result,schema:result?.schema||SCHEMA,knowledgeAware:false,coDevelopmentAware:true,simulationTrained:true}}
function status(){let base={configured:true,ready:false,mode:'public-native-loading',engine:'gthink-public-native-js+core-cognition+co-development+sim-trained',model:'simulation-trained-conversation-v4',offlineCapable:true,networkRequired:false,knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true,simulationTrained:true};try{if(coreRef?.status)base={...base,...coreRef.status(),ready:true,engine:'gthink-public-native-js+core-cognition+co-development+sim-trained',model:'simulation-trained-conversation-v4',knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true,simulationTrained:true,conversationBridge:!!conversationRef,conversationBridgeSchema:conversationRef?.schema||null}}catch{}return base}
wrapper=Object.freeze({schema:SCHEMA,name:'GThinkPublicNativeSimulationTrained',answer,status,get core(){return coreRef},get conversation(){return conversationRef},ready:()=>readyPromise});
window.GTHINK_PUBLIC_NATIVE_ENGINE=wrapper;
})();