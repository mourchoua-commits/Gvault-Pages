(()=>{'use strict';
const SCHEMA='GTHINK_PUBLIC_NATIVE_ENGINE_V4_CO_DEVELOPMENT';
...
const CONVERSATION_URL=new URL('gthink-secondary-conversation-bridge.js?v=3',SCRIPT_BASE).href;
...
const readyPromise=Promise.all([
  ...
  loadScript(CONVERSATION_URL,'data-gthink-secondary-conversation-bridge-v3')
]).then(()=>{
...
async function answer(request){const {core,conversation}=await readyPromise;const context=request?.payload?.secondKernelContext||null;let conversational=null;try{conversational=conversation?.answer?.(request,context)||null}catch{}if(conversational?.handled&&String(conversational?.text||'').trim())return {schema:SCHEMA,handled:true,text:String(conversational.text),engine:'gthink-public-native-js+core-cognition+co-development',model:'co-development-conversation-v3',publicNative:true,offlineCapable:true,actionsAuthorized:false,conversationIntent:conversational.intent||null,evidence:conversational.evidence||[],knowledgeBranch:conversational.knowledgeBranch||null,knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true};const result=await core.answer(request);return typeof result==='string'?{schema:SCHEMA,handled:true,text:result,engine:'gthink-public-native-js',model:'native-rules-v1',publicNative:true,offlineCapable:true,actionsAuthorized:false,knowledgeAware:false,coDevelopmentAware:true}:{...result,schema:result?.schema||SCHEMA,knowledgeAware:false,coDevelopmentAware:true}}
function status(){let base={configured:true,ready:false,mode:'public-native-loading',engine:'gthink-public-native-js+core-cognition+co-development',model:'co-development-conversation-v3',offlineCapable:true,networkRequired:false,knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true};try{if(coreRef?.status)base={...base,...coreRef.status(),ready:true,engine:'gthink-public-native-js+core-cognition+co-development',model:'co-development-conversation-v3',knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true,conversationBridge:!!conversationRef,conversationBridgeSchema:conversationRef?.schema||null}}catch{}return base}
wrapper=Object.freeze({schema:SCHEMA,name:'GThinkPublicNativeCoDevelopment',answer,status,get core(){return coreRef},get conversation(){return conversationRef},ready:()=>readyPromise});
window.GTHINK_PUBLIC_NATIVE_ENGINE=wrapper;
})();