(()=>{'use strict';
const SCHEMA='GTHINK_PUBLIC_NATIVE_ENGINE_V7_FREE_CONVERSATION_GUARD';
...
function status(){let base={configured:true,ready:false,mode:'public-native-loading',engine:'gthink-public-native-js+core-cognition+co-development+free-conversation+intent-router',model:'conversation-guard-v5',offlineCapable:true,networkRequired:false,knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true,simulationTrained:true,testIntentRouted:true,freeConversationGuard:true};try{if(coreRef?.status)base={...base,...coreRef.status(),ready:true,engine:'gthink-public-native-js+core-cognition+co-development+free-conversation+intent-router',model:'conversation-guard-v5',knowledgeAware:true,coreCognitionAware:true,coDevelopmentAware:true,simulationTrained:true,testIntentRouted:true,freeConversationGuard:true,testIntentRouterSchema:testIntentRef?.schema||null,conversationBridge:!!conversationRef,conversationBridgeSchema:conversationRef?.schema||null}}catch{}return base}
wrapper=Object.freeze({schema:SCHEMA,name:'GThinkPublicNativeFreeConversationGuard',answer,status,get core(){return coreRef},get testIntent(){return testIntentRef},get conversation(){return conversationRef},ready:()=>readyPromise});
window.GTHINK_PUBLIC_NATIVE_ENGINE=wrapper;
})();