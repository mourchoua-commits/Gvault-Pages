...
const gate=await publicCaptureGate({root:process.cwd(),outDir,rawBytes,eventKind:'AI_PUBLIC_OUTBOUND_REQUEST',eventId:String(packet.packetId),historyKey:String(packet.payloadSha256)});
const result={schema:'GVAULT_AI_PUBLIC_INGRESS_RESULT_V2',status:gate.status,changed:gate.historyCreated||gate.indexCreated||gate.pointerUpdated||gate.pointerRestored,historyCreated:gate.historyCreated,pointerRestored:gate.pointerRestored,replayed:gate.replayed,packetId:packet.packetId,payloadSha256:packet.payloadSha256,captureSha256:gate.captureSha256,utf8Bytes:gate.utf8Bytes,historyPath:gate.historyPath,currentPath:gate.currentPath,indexPath:gate.indexPath,historyFirst:true,readbackExact:gate.readbackExact,indexStatus:gate.indexStatus,publicCaptureGate:gate};
