(()=>{'use strict';
const SCHEMA='GVAULT_BLOB_URI_V1';
const PREFIX='blob://';
...
function publicHref(path){const clean=String(path||'').replace(/^\/+/, '');if(/(^|\/)\.\.?($|\/)/.test(clean))return null;const url=new URL(clean||'./',SITE_ROOT);if(url.origin!==location.origin)return null;if(!url.pathname.startsWith(SITE_ROOT.pathname))return null;return url.href}
function resolve(uri){const p=parse(uri);if(!p.ok)return p;if(p.scope==='public'){const href=publicHref(p.path);return href?Object.freeze({ok:true,schema:SCHEMA,uri:p.raw,scope:p.scope,kind:'public-url',href}):fail(p.raw,'public_path_denied')}
