...
export async function produce(payload){const packet=await sealAgentPublicOutbound(payload);await verifyAgentPublicOutbound(packet);if(writer){const result=await writer(structuredClone(packet));emit('gvault:ai-public-produced',{packetId:packet.packetId,result});return {status:'DISPATCHED_TO_TRUSTED_WRITER',packet,result}}const out=load(OUTBOX_KEY);if(!out.some(x=>x.packetId===packet.packetId)){out.push(packet);save(OUTBOX_KEY,out)}emit('gvault:ai-public-outbox',{packetId:packet.packetId,status:'SEALED_AWAITING_TRUSTED_WRITER'});return {status:'SEALED_AWAITING_TRUSTED_WRITER',packet}}
