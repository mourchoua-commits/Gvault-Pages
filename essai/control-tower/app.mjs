import {normalizeEvent,sortEvents,dedupeEvents,buildTrack,applyEventsToTracks,extractAnchors,safeString} from './core.mjs';
...
const LOCAL_TRACK_KEY='gvault.controlTower.tracks.v1';
const POLL_MS=30000;
...
async function saveRemoteTracks(){
  const payload=JSON.stringify({schema:'GVAULT_CONTROL_TOWER_TRACK_REGISTRY_V1',updatedAt:new Date().toISOString(),tracks},null,2)+'\n';
...
function renderEvents(){const x=filteredEvents();$('#eventCount').textContent=x.length;$('#eventList').innerHTML=x.slice(0,500).map((e,i)=>`<article class="event ${e.severity} ${selected?.id===e.id?'selected':''}" data-eid="${esc(e.id)}"><button class="eventMain" data-open="${esc(e.id)}"><div class="eventTop"><span class="sev">${severityGlyph(e.severity)}</span><b>${esc(e.engine)}</b><time>${esc(formatTime(e.at))}</time></div><div class="summary">${esc(e.summary)}</div><div class="meta">${esc(e.type)}${e.status?' · '+esc(e.status):''}${e.project?' · '+esc(e.project):''}</div></button><button class="tagBtn" title="Tag / suivre" data-tag="${esc(e.id)}">⌖</button></article>`).join('')||'<div class="empty">aucun événement pour ce filtre</div>';
  document.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>selectEvent(b.dataset.open));document.querySelectorAll('[data-tag]').forEach(b=>b.onclick=e=>{e.stopPropagation();openTrackDialog(b.dataset.tag)});
}
...
function renderDetail(){const e=selected;if(!e){$('#detail').innerHTML='<div class="empty big">Sélectionne un événement pour descendre jusqu’à sa preuve.</div>';return}const anchors=extractAnchors(e);$('#detail').innerHTML=`<div class="detailHead"><div><span class="badge ${e.severity}">${esc(e.severity)}</span><h2>${esc(e.engine)}</h2></div><button id="detailTrack">TAG / SUIVRE</button></div><h3>${esc(e.summary)}</h3><dl><dt>ID</dt><dd>${esc(e.id)}</dd><dt>TYPE</dt><dd>${esc(e.type)}</dd><dt>STATUT</dt><dd>${esc(e.status||'—')}</dd><dt>DATE</dt><dd>${esc(formatTime(e.at))}</dd><dt>SHA</dt><dd>${esc(e.sha||'—')}</dd></dl><section><b>ANCRES DE SUIVI DÉTECTÉES</b><div class="chips">${anchors.map(a=>`<span>${esc(a.key)}=${esc(a.value)}</span>`).join('')||'<i>aucune ancre stable</i>'}</div></section><section><b>PREUVES</b>${(e.proofRefs||[]).map(p=>`<div class="proof">${esc(p)}</div>`).join('')||'<i>aucune référence explicite</i>'}</section><section><b>ÉVÉNEMENT BRUT</b><pre>${esc(JSON.stringify(e.raw,null,2))}</pre></section>`;$('#detailTrack').onclick=()=>openTrackDialog(e.id)}
function renderTracks(){const box=$('#trackList');box.innerHTML=tracks.map(t=>`<article class="track"><div class="trackTop"><b>${esc(t.trackId)}</b><span>${esc(t.syncState||'LOCAL')}</span></div><h3>${esc(t.label)}</h3><div class="trackMeta">${esc(t.engine)} · ${(t.matches||[]).length} événement(s) · ${esc(t.state)}</div><div class="chips">${(t.anchors||[]).map(a=>`<span>${esc(a.key)}=${esc(a.value)}</span>`).join('')}</div><div class="trackActions"><button data-showtrack="${esc(t.trackId)}">TIMELINE</button><button data-paused="${esc(t.trackId)}">${t.state==='ACTIVE'?'PAUSE':'REPRENDRE'}</button></div></article>`).join('')||'<div class="empty">aucune information taguée</div>';document.querySelectorAll('[data-showtrack]').forEach(b=>b.onclick=()=>showTrack(b.dataset.showtrack));document.querySelectorAll('[data-paused]').forEach(b=>b.onclick=()=>toggleTrack(b.dataset.paused))}
function showTrack(id){const t=tracks.find(x=>x.trackId===id);if(!t)return;$('#trackTimelineTitle').textContent=`${t.trackId} · ${t.label}`;$('#trackTimeline').innerHTML=(t.matches||[]).slice().sort((a,b)=>(Date.parse(b.at)||0)-(Date.parse(a.at)||0)).map(m=>`<div class="timelineItem"><time>${esc(formatTime(m.at))}</time><b>${esc(m.engine)} · ${esc(m.type)}</b><p>${esc(m.summary)}</p><small>${esc(m.sha||m.eventId)}</small></div>`).join('')||'<div class="empty">aucun match</div>';$('#trackModal').showModal()}
async function toggleTrack(id){const t=tracks.find(x=>x.trackId===id);if(!t)return;t.state=t.state==='ACTIVE'?'PAUSED':'ACTIVE';t.updatedAt=new Date().toISOString();t.syncState='LOCAL_PENDING';renderTracks();await syncTracks()}
...
function runCommand(line){const [cmd,...args]=line.trim().split(/\s+/);if(!cmd)return;const q=args.join(' ');log('> '+line,'cmd');switch(cmd.toLowerCase()){
  case 'help':log('help · live · alerts · tracks · engines · search <texte> · engine <nom> · track <TRACK-ID> · refresh · status · clear');break;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  window.GVAULT_CONTROL_TOWER={schema:'GVAULT_CONTROL_TOWER_V1',setToken:async t=>{token=String(t||'');await refresh()},refresh,lock,getState:()=>({events:events.length,tracks:tracks.length,lastRefresh})};
  window.addEventListener('pagehide',()=>{token='';if(pollTimer)clearInterval(pollTimer)});
  log('CONTROL TOWER prêt · ouvre le SAS pour lire le Gvault privé');
});
