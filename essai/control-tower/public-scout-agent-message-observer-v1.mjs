...
const OWNER='mourchoua-commits',REPO='Gvault-Pages',BRANCH='experiment/power-ranger-public-scout-20260830';
...
async function verifyMessage(state,ack){
...
export async function readVerifiedAgentPublicMessage(){
  ...
  const ar=await fetch(ackUrl,{cache:'no-store'});if(!ar.ok)throw new Error(`AI_MESSAGE_ACK_HTTP_${ar.status}`);const ack=await ar.json();await verifyAck(ack);
  const state=await fetchExactMessage(ack.dataCommitSha);await verifyMessage(state,ack);
...
window.GVAULT_AI_PUBLIC_MESSAGE_OBSERVER_V1=Object.freeze({schema:'GVAULT_AI_PUBLIC_MESSAGE_OBSERVER_V1',readVerifiedAgentPublicMessage,startAgentPublicMessageObserver});
