...
async function localByPayload(rel){const rows=await localSnaps(),match=String(rel).match(/([a-f0-9]{64})/i),payloadHash=match?match[1]:'';const x=rows.find(s=>String(s.manifest?.payload?.path||'')===String(rel)||(payloadHash&&String(s.payloadSha256||'')===payloadHash));if(!x)throw new Error('LOCAL_VFS_PAYLOAD_NOT_FOUND');return x}
