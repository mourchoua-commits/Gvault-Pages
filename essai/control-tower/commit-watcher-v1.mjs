...
const PROPAGATION_RETRY_MS=30000;
...
function setLast(commit,detectedAt=new Date().toISOString()){const x={schema:'GVAULT_CONTROL_TOWER_COMMIT_WATCH_HEAD_V1',sha:commit.sha,snapshotPrefix:commit.snapshotPrefix||null,detectedAt,path:WATCH_PATH};writeJson(LAST,x);lastCommit=commit.sha;return x}
...
function statusHost(){return document.querySelector('#ctWatcherStateHost')||document.querySelector('#ctVfs .ctVfsHead')||document.querySelector('.brand')||null}
...
async function fetchLatestCommit(){const r=await fetch(API,{cache:'no-store',credentials:'omit',headers:{'Accept':'application/vnd.github+json'}});remaining=Number(r.headers.get('x-ratelimit-remaining'));const reset=Number(r.headers.get('x-ratelimit-reset'));resetAt=Number.isFinite(reset)&&reset>0?new Date(reset*1000).toISOString():null;if(r.status===403||r.status===429){const e=new Error('RATE_LIMIT');e.status='RATE_LIMIT';e.retryAfter=retryDelayMs({status:'RATE_LIMIT',rateResetEpochSec:reset,nowMs:now(),errorMs:ERROR_MS});throw e}if(!r.ok)throw new Error('HTTP_'+r.status);const rows=await r.json();if(!Array.isArray(rows)||!rows.length)return null;return parseSnapshotCommit(rows[0])}
async function triggerSync(commit,previousSha){emit('gvault:control-tower-public-commit',{sha:commit.sha,previousSha,snapshotPrefix:commit.snapshotPrefix});const api=window.GVAULT_CONTROL_TOWER_VFS_V2;if(!api?.syncChain){const e=new Error('VFS_NOT_READY');e.status='VFS_NOT_READY';e.retryAfter=retryDelayMs({status:e.status,propagationMs:PROPAGATION_RETRY_MS,errorMs:ERROR_MS});throw e}try{const result=await api.syncChain({source:'commit-watcher',commitSha:commit.sha});const verdict=classifyIngestion({previousSha,commit,vfsResult:result});if(!verdict.ack){const e=new Error(verdict.status);e.status=verdict.status;e.retryAfter=retryDelayMs({status:verdict.status,propagationMs:PROPAGATION_RETRY_MS,errorMs:ERROR_MS});throw e}emit('gvault:control-tower-vfs-ingested',{sha:commit.sha,previousSha,snapshotPrefix:commit.snapshotPrefix,result,verdict});return {result,verdict}}catch(e){emit('gvault:control-tower-vfs-ingest-failed',{sha:commit.sha,previousSha,snapshotPrefix:commit.snapshotPrefix,error:String(e?.message||e),status:e?.status||null});throw e}}
async function check(force=false){if(stopped)return;if(!force&&!acquireLeader()){renderStatus();return}lastCheckAt=now();try{const commit=await fetchLatestCommit();lastError='';if(!commit){emit('gvault:control-tower-commit-watch-empty');renderStatus();return}const prev=readLast()?.sha||'';lastCommit=commit.sha;if(prev!==commit.sha){emit('gvault:control-tower-new-public-commit',{sha:commit.sha,previousSha:prev||null,snapshotPrefix:commit.snapshotPrefix});await triggerSync(commit,prev||null);setLast(commit)}else emit('gvault:control-tower-public-commit-same',{sha:commit.sha,snapshotPrefix:commit.snapshotPrefix});renderStatus()}catch(e){lastError=String(e?.message||e);emit('gvault:control-tower-commit-watch-error',{error:lastError,status:e?.status||null});renderStatus();throw e}}
