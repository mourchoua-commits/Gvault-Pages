...
const validPrefix=s=>/^[a-f0-9]{12,64}$/i.test(String(s||''));
const now=()=>Date.now();
...
async function fetchLatestCommit(){const r=await fetch(API,{cache:'no-store',credentials:'omit',headers:{'Accept':'application/vnd.github+json'}});remaining=Number(r.headers.get('x-ratelimit-remaining'));const reset=Number(r.headers.get('x-ratelimit-reset'));resetAt=Number.isFinite(reset)&&reset>0?new Date(reset*1000).toISOString():null;if(r.status===403||r.status===429){const e=new Error('RATE_LIMIT');e.retryAfter=Math.max(ERROR_MS,(reset*1000)-now()+5000);throw e}if(!r.ok)throw new Error('HTTP_'+r.status);const rows=await r.json();if(!Array.isArray(rows)||!rows.length)return null;const sha=String(rows[0]?.sha||'');if(!validSha(sha))return null;const message=String(rows[0]?.commit?.message||'');const prefix=(message.match(/observability:\s*snapshot\s+([a-f0-9]{12,64})/i)||[])[1]||'';return {sha,message,snapshotPrefix:validPrefix(prefix)?prefix.toLowerCase():null}}
async function triggerSync(commit,previousSha){emit('gvault:control-tower-public-commit',{sha:commit.sha,previousSha,snapshotPrefix:commit.snapshotPrefix});const api=window.GVAULT_CONTROL_TOWER_VFS_V2;if(!api?.syncChain){const e=new Error('VFS_NOT_READY');e.retryAfter=PROPAGATION_RETRY_MS;throw e}try{const result=await api.syncChain({source:'commit-watcher',commitSha:commit.sha});if(!result?.ok){const e=new Error(result?.busy?'VFS_BUSY':result?.error||'VFS_SYNC_FAILED');e.retryAfter=PROPAGATION_RETRY_MS;throw e}if(commit.snapshotPrefix&&(!result.headChain||!String(result.headChain).toLowerCase().startsWith(commit.snapshotPrefix))){const e=new Error('PAGES_HEAD_NOT_PROPAGATED');e.retryAfter=PROPAGATION_RETRY_MS;throw e}emit('gvault:control-tower-vfs-ingested',{sha:commit.sha,previousSha,snapshotPrefix:commit.snapshotPrefix,result});return result}catch(e){emit('gvault:control-tower-vfs-ingest-failed',{sha:commit.sha,previousSha,snapshotPrefix:commit.snapshotPrefix,error:String(e?.message||e)});throw e}}
async function check(force=false){if(stopped)return;if(!force&&!acquireLeader()){renderStatus();return}lastCheckAt=now();try{const commit=await fetchLatestCommit();lastError='';if(!commit){emit('gvault:control-tower-commit-watch-empty');renderStatus();return}const prev=readLast()?.sha||'';lastCommit=commit.sha;if(prev!==commit.sha){emit('gvault:control-tower-new-public-commit',{sha:commit.sha,previousSha:prev||null,snapshotPrefix:commit.snapshotPrefix});await triggerSync(commit,prev||null);setLast(commit)}else emit('gvault:control-tower-public-commit-same',{sha:commit.sha,snapshotPrefix:commit.snapshotPrefix});renderStatus()}catch(e){lastError=String(e?.message||e);emit('gvault:control-tower-commit-watch-error',{error:lastError});renderStatus();throw e}}
