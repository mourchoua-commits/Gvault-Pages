const VERSION='gvault-shell-v1-20260902-gthink-offline-v1';
...
const isGThinkCritical=url=>url.origin===scopeOrigin&&/(?:^|\/)(?:gvault-agent-live-blob|gthink-mini-listener-swarm|gthink-prelistener-stream-blob|gthink-throughput-guard|gthink-public-responder|gthink-turn-relay|gvault-person-blob)\.js$/i.test(url.pathname);
...
async function cacheFirst(request,cacheName=SHELL_CACHE){const old=await cached(cacheName,request);if(old)return old;const r=await fetch(request,{cache:'no-store'});if(isGood(r))void putSafe(cacheName,request,r);return r}
...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('gvault-shell-v1-')&&!k.startsWith(VERSION)).map(k=>caches.delete(k)));await self.clients.claim();await announce('READY',{version:VERSION,inputRelay:'GVAULT_PUBLIC_INPUT_RELAY_V3',inputRelayMode:'EXPLICIT_ONLY',blobFallback:'DURABLE_LOCAL_QUEUE',agentLiveBlob:'GVAULT_AGENT_LIVE_BLOB_CLIENT_V6',gthinkMiniSwarm:'GTHINK_MINI_LISTENER_SWARM_V3_OFFLINE',gthinkThroughputGuard:'GTHINK_THROUGHPUT_GUARD_V2',gthinkOfflineOnly:true})})())});
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(isPrivateData(url)||isAuthCritical(url))return;if(isGitHubPublicApi(url)){event.respondWith(networkFirst(req,API_CACHE,7000));return}if(isBaseline(url)){event.respondWith(networkFirst(req,SHELL_CACHE,5000));return}if(url.origin!==scopeOrigin)return;if(req.mode==='navigate'){if(isGThinkPath(url)){event.respondWith(cacheFirst(req,SHELL_CACHE));return}event.respondWith(networkFirst(req,SHELL_CACHE,3500).then(injectInputRelay));return}if(isGThinkCritical(url)){event.respondWith(cacheFirst(req,SHELL_CACHE));return}if(['script','style','worker','font'].includes(req.destination)){event.respondWith(staleWhileRevalidate(req));return}event.respondWith(networkFirst(req,SHELL_CACHE,4500))});
self.addEventListener('message',event=>{const d=event.data||{};if(d.schema!=='GVAULT_SW_COMMAND_V1')return;if(d.command==='STATUS')event.source?.postMessage({schema:'GVAULT_SW_STATUS_V1',version:VERSION,scope:SCOPE,inputRelay:'GVAULT_PUBLIC_INPUT_RELAY_V3',inputRelayMode:'EXPLICIT_ONLY',blobFallback:'DURABLE_LOCAL_QUEUE',agentLiveBlob:'GVAULT_AGENT_LIVE_BLOB_CLIENT_V6',gthinkMiniSwarm:'GTHINK_MINI_LISTENER_SWARM_V3_OFFLINE',gthinkThroughputGuard:'GTHINK_THROUGHPUT_GUARD_V2',gthinkOfflineOnly:true});if(d.command==='REFRESH_SHELL')event.waitUntil((async()=>{for(const url of SHELL){try{const r=await fetch(url,{cache:'reload'});if(isGood(r))await putSafe(SHELL_CACHE,url,r)}catch{}}await announce('SHELL_REFRESHED',{version:VERSION})})())});