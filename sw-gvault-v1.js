const VERSION='gvault-shell-v1-20260902-gthink-public-private-v1';
...
const isGThinkCritical=url=>url.origin===scopeOrigin&&/(?:^|\/)(?:gvault-agent-live-blob|gthink-public-private-bridge|gthink-mini-listener-swarm|gthink-prelistener-stream-blob|gthink-throughput-guard|gthink-public-responder|gthink-turn-relay|gvault-person-blob)\.js$/i.test(url.pathname);
...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('gvault-shell-v1-')&&!k.startsWith(VERSION)).map(k=>caches.delete(k)));await self.clients.claim();await announce('READY',{version:VERSION,inputRelay:'GVAULT_PUBLIC_INPUT_RELAY_V3',inputRelayMode:'EXPLICIT_ONLY',blobFallback:'DURABLE_LOCAL_QUEUE',agentLiveBlob:'GVAULT_AGENT_LIVE_BLOB_CLIENT_V6',gthinkMiniSwarm:'GTHINK_MINI_LISTENER_SWARM_V5_PRIVATE_RELAY',gthinkThroughputGuard:'GTHINK_THROUGHPUT_GUARD_V3_PRIVATE_BRIDGE',gthinkPublicPrivateBridge:'GTHINK_PUBLIC_PRIVATE_BRIDGE_CLIENT_V1',gthinkOfflineOnly:false})})())});
...
self.addEventListener('message',event=>{const d=event.data||{};if(d.schema!=='GVAULT_SW_COMMAND_V1')return;if(d.command==='STATUS')event.source?.postMessage({schema:'GVAULT_SW_STATUS_V1',version:VERSION,scope:SCOPE,inputRelay:'GVAULT_PUBLIC_INPUT_RELAY_V3',inputRelayMode:'EXPLICIT_ONLY',blobFallback:'DURABLE_LOCAL_QUEUE',agentLiveBlob:'GVAULT_AGENT_LIVE_BLOB_CLIENT_V6',gthinkMiniSwarm:'GTHINK_MINI_LISTENER_SWARM_V5_PRIVATE_RELAY',gthinkThroughputGuard:'GTHINK_THROUGHPUT_GUARD_V3_PRIVATE_BRIDGE',gthinkPublicPrivateBridge:'GTHINK_PUBLIC_PRIVATE_BRIDGE_CLIENT_V1',gthinkOfflineOnly:false});if(d.command==='REFRESH_SHELL')event.waitUntil((async()=>{for(const url of SHELL){try{const r=await fetch(url,{cache:'reload'});if(isGood(r))await putSafe(SHELL_CACHE,url,r)}catch{}}await announce('SHELL_REFRESHED',{version:VERSION})})())});
