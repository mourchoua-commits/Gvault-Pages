const VERSION='gvault-shell-v1-20260902-private-ubiquity-v1';
...
const SHELL=[
  './','./index.html','./scripts/gvault-input-relay.js','./scripts/gvault-input-relay-key.v2.json','./scripts/gvault-public-agent-conversation.js','./scripts/gvault-private-ubiquity.js',
  ...GTHINK_SHELL,
...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('gvault-shell-v1-')&&!k.startsWith(VERSION)).map(k=>caches.delete(k)));await self.clients.claim();await announce('READY',{version:VERSION,inputRelay:'GVAULT_PUBLIC_INPUT_RELAY_V3',inputRelayMode:'EXPLICIT_ONLY',privateUbiquity:'GVAULT_PRIVATE_UBIQUITY_MESH_V1',privateUbiquityScope:'ALL_PUBLIC_HTML',privateContentPublished:false,blobFallback:'DURABLE_LOCAL_QUEUE',agentLiveBlob:'GVAULT_AGENT_LIVE_BLOB_CLIENT_V6',gthinkPublicNativeEngine:'GTHINK_PUBLIC_NATIVE_ENGINE_V1',gthinkMiniSwarm:'GTHINK_MINI_LISTENER_SWARM_V7_PUBLIC_NATIVE_RELAY',gthinkThroughputGuard:'GTHINK_THROUGHPUT_GUARD_V3_PRIVATE_BRIDGE',gthinkPublicPrivateBridge:'GTHINK_PUBLIC_PRIVATE_BRIDGE_CLIENT_V5_DIRECT_REMOTE',gthinkClientPrivateWorker:'GTHINK_CLIENT_PRIVATE_WORKER_V1',gthinkPagePrelistenerMesh:'GTHINK_PAGE_PRELISTENER_MESH_V2_NEAREST_SOURCE',gthinkOfflineOnly:true})})())});
...
self.addEventListener('message',event=>{const d=event.data||{};if(d.schema!=='GVAULT_SW_COMMAND_V1')return;if(d.command==='STATUS')event.source?.postMessage({schema:'GVAULT_SW_STATUS_V1',version:VERSION,scope:SCOPE,inputRelay:'GVAULT_PUBLIC_INPUT_RELAY_V3',inputRelayMode:'EXPLICIT_ONLY',privateUbiquity:'GVAULT_PRIVATE_UBIQUITY_MESH_V1',privateUbiquityScope:'ALL_PUBLIC_HTML',privateContentPublished:false,blobFallback:'DURABLE_LOCAL_QUEUE',agentLiveBlob:'GVAULT_AGENT_LIVE_BLOB_CLIENT_V6',gthinkPublicNativeEngine:'GTHINK_PUBLIC_NATIVE_ENGINE_V1',gthinkMiniSwarm:'GTHINK_MINI_LISTENER_SWARM_V7_PUBLIC_NATIVE_RELAY',gthinkThroughputGuard:'GTHINK_THROUGHPUT_GUARD_V3_PRIVATE_BRIDGE',gthinkPublicPrivateBridge:'GTHINK_PUBLIC_PRIVATE_BRIDGE_CLIENT_V5_DIRECT_REMOTE',gthinkClientPrivateWorker:'GTHINK_CLIENT_PRIVATE_WORKER_V1',gthinkPagePrelistenerMesh:'GTHINK_PAGE_PRELISTENER_MESH_V2_NEAREST_SOURCE',gthinkOfflineOnly:true});if(d.command==='REFRESH_SHELL')event.waitUntil((async()=>{for(const url of SHELL){try{const r=await fetch(url,{cache:'reload'});if(isGood(r))await putSafe(SHELL_CACHE,url,r)}catch{}}await announce('SHELL_REFRESHED',{version:VERSION})})())});
