...
function errorDocument(message,terminal){
  ...
  return `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Blob route indisponible</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#05070b;color:#fff;font:16px system-ui;padding:24px}main{max-width:560px}a{color:#9fc0ff}code{font-family:ui-monospace,monospace}</style><main><h1>Route blob</h1><p>${String(message||'Erreur de route')}</p>${terminalLine}<p><a href="${direct}">Ouvrir GThink directement</a></p></main>`;
}
...
async function serveVirtual(request){
  ...
  if(status===404){
    ...
    return new Response(errorDocument('Arrêt au premier HTTP 404 · la cible suivante est introuvable.',TERMINAL_404),{status:404,statusText:'Not Found',headers:headersFor(routeHops,404,TERMINAL_404)});
  }
  if(!response.ok){
    return new Response(errorDocument('GThink HTTP '+status),{status:502,headers:headersFor(hops,status,'http-error-blob')});
  }
